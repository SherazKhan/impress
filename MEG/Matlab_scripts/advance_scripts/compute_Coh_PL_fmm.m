function compute_Coh_PL_fmm(subj,visitNo,freq,label_name,nepochs,label_dir)
...
fname_inv = strcat('/autofs/cluster/transcend/fmm/',subj,'/',num2str(visitNo),'/',subj,'_fmm_0.1_140_calc-inverse_fixed_ico4_weight_new_erm_megreg_0_new_MNE_proj-inv.fif');
inv = mne_read_inverse_operator(fname_inv);

% standard condition epochs
nepochs_standard=nepochs*2;
...
c1.cortex=c1.cortex(:,:,1:nepochs_standard);
...
save([data_dir 'Time_Freq/' subj '_cortex_inv_fixed_all_sensors_ds_cond1_cond2_freq_' label_name(1:end-6) '_' num2str(freq(1)) '-' num2str(freq(end)) '_' num2str(nepochs) 'epochs.mat'],...
    'PL_B','PL_A','Coh_B','Coh_A','Z_PL','Z_Coh','freq','label_name','nepochs_A','nepochs_B');

stemPL=[data_dir 'Time_Freq/' subj '_cortex_inv_fixed_all_sensors_ds_cond1_cond2_freq_PL_' label_name(1:end-6) '_' num2str(freq(1)) '-' num2str(freq(end)) '-' num2str(nepochs) 'epochs'];
stemCoh=[data_dir 'Time_Freq/' subj '_cortex_inv_fixed_all_sensors_ds_cond1_cond2_freq_Coh_' label_name(1:end-6) '_' num2str(freq(1)) '-' num2str(freq(end)) '-' num2str(nepochs) 'epochs'];
