function [modidx_canolty, modidx_tort]=cfc(data,fs,phase_freqs,amplitude_freqs)
...
for lower_fi=1:length(phase_freqs)
    ...
    low_filt=eegfiltfft(data,fs,phase_freqs(lower_fi)-.5,phase_freqs(lower_fi)+.5);
    phase=angle(hilbert(low_filt));
    
    for upper_fi=1:length(amplitude_freqs)
        
        % now get upper power time series
        hi_filt=eegfiltfft(data,fs,amplitude_freqs(upper_fi)-5,amplitude_freqs(upper_fi)+5);
