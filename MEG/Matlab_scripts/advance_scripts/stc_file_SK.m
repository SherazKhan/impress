function stc_file_SK(cfg,subj,visitNo,run,nepochs)
...
for i=1:cfg.condnum
    ...
    stem  = [cfg.data_rootdir,'/',subj,'/',num2str(visitNo),'/',subj,'_',cfg.protocol,'_','cond_' num2str(i) '_run_' num2str(run) '_' num2str(cfg.highpass) '_' num2str(cfg.lowpass) 'fil']; % Stem for the stc files
    invop = mne_read_inverse_operator(fname_inv);    % The inverse operator *structure* (can be the forward operator as well)
    
    tmin  = D.evoked(1,i).times(1);       % minimum time
    tstep = 1/D.info.sfreq; % 
    mne_write_inverse_sol_stc(stem,invop,double(PALL),tmin,tstep)
    
    stc_out=[cfg.data_rootdir '/' subj '/' num2str(visitNo) '/' subj '_' cfg.protocol '_','cond_' num2str(i) '_run_' num2str(run) '_'  num2str(cfg.highpass) '_' num2str(cfg.lowpass) 'fil-morph' ];
